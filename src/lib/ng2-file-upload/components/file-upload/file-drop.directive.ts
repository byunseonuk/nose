import { Component, EventEmitter, ElementRef, HostListener, Input, Output, OnInit } from '@angular/core';

import { FileUploader } from './file-uploader.class';
@Component({
  selector: '[ng2FileDrop]',
  template: `<div><input multiple class="drop-click-handler" type='file'/> <ng-content></ng-content></div>`
})
export class FileDropDirective implements OnInit{
  ...
  ngOnInit(): void {
    this.inputElement = this.element.nativeElement.querySelector('input.drop-click-handler');
  ...
  public fileSelectHandler(event:any){
    ...
    this.uploader.addToQueue(event.target.files, options, filters);
